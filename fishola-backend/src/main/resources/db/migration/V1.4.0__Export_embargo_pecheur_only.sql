-- Embargo d'export : ne s'applique qu'à la saisie pêcheur.
--
-- Les vues d'export masquaient toute sortie créée depuis moins de 168 h. Ce délai
-- existe pour laisser au PÊCHEUR le temps de corriger sa sortie (même fenêtre que
-- « modifiableUntil ») avant qu'elle ne parte à l'export.
--
-- Les données recueillies par un OPÉRATEUR (import CSV, saisie manuelle : méthodes
-- « enquete », « carnet_volontaire », « carnet_obligatoire ») ne sont jamais amendées
-- par un pêcheur : l'embargo n'y protégeait rien et rendait simplement le travail de
-- l'opérateur invisible pendant une semaine, y compris pour le contrôler après import.
--
-- Seule la clause d'embargo change ; le reste des vues est repris à l'identique.
