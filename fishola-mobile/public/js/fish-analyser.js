      ...
      imgElement.onload = function() {
       ...
       /* var borderValue = cv.Scalar.all(Number.MAX_VALUE);
        ...
        cv.findContours(output2, contours, hierarchy, cv.RETR_CCOMP, cv.CHAIN_APPROX_SIMPLE);
        // draw contours with random Scalar
        for (let i = 0; i < contours.size(); ++i) {
            let color = new cv.Scalar(Math.round(Math.random() * 255), Math.round(Math.random() * 255),
                                    Math.round(Math.random() * 255));
            cv.drawContours(output2, contours, i, color, 1, cv.LINE_8, hierarchy, 100);
        }*/
        ...
        cv.imshow('canvasOutput', output2);
        src.delete();
        dst.delete();
      };