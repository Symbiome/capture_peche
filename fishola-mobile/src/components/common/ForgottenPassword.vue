...
<template>
  <div class="forgotten-password keyboardSensitive" v-bind:class="{ expanded: !collapsed, collapsed: collapsed }">
    ...
    <div v-if="!collapsed">
      <h1 class="title">Mot de passe oublié {{ newPassword }} {{ newPasswordConfirmation }}</h1>
      <FormInput name="forgottenEmail" type="email" label="Pour quel e-mail avez-vous oublié votre mot de passe ?"
        placeholder="Renseignez votre E-mail" v-model="forgottenEmail" v-bind:error="emailError"
        v-on:keyupEnter="sendPasswordReinitialisationRequest" />
      <FormInput name="newPassword" type="password" label="Choisissez un nouveau mot de passe"
        placeholder="Nouveau mot de passe" v-model="newPassword" v-bind:error="passwordError"
        v-on:keyupEnter="sendPasswordReinitialisationRequest" />
      <FormInput name="newPasswordConfirmation" type="password" label="Confirmez votre nouveau mot de passe"
        placeholder="Nouveau mot de passe" v-model="newPasswordConfirmation" v-bind:error="passwordConfirmationError"
        v-on:keyupEnter="sendPasswordReinitialisationRequest" />
      <div class="sendpassword">
        <button class="cancel" v-on:click="cancel">Annuler</button>
