...
export default class ProfileService extends AbstractFisholaService {
  ...
  static fetchProfile(): Promise<UserProfile> {
    return new Promise<UserProfile>((resolve, reject) => {
      this.backendGetOrOfflineStorage("/v1/security/profile").then(
        (fetched) => {
  ...
  static saveProfile(profile: UserProfile): Promise<void> {
    return new Promise((resolve, reject) => {
      this.backendPut("/v1/security/profile", profile).then(() => {
  ...
  static resetPassword(token: string) {
    ...
    return this.backendGetWithArgs(
      "/v1/security/reset-password-app",
      urlParams
    );
  }
    ...
    this.backendPut("/v1/security/register", newUser).then(
      ...
      (error) => {
        if (error.status == 400) {
          ...
          console.error(
            "Error in httpCall reponse " + error.status,
            error["responseText"]
          );
  ...
  static fetchSettings(): Promise<UserSettings> {
    const promise = new Promise<UserSettings>((resolve, reject) => {
      this.backendGetOrOfflineStorage("/v1/security/settings").then(
        (fetched) => {
  ...
  static saveSettings(settings: UserSettings): Promise<void> {
    return new Promise((resolve, reject) => {
      this.backendPut("/v1/security/settings", settings).then(() => {
